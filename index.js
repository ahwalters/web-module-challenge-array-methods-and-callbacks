...
function getAverageGoals(getFinalsCB) {
    const teams = getFinalsCB.length;
    let goals = getFinalsCB.reduce((accumulator, currentValue) => {
        return accumulator += currentValue["Home Team Goals"] + currentValue["Away Team Goals"];
    }, 0)
    return (goals/teams).toFixed(2);
};
...
function getCountryWins(data, teamInitials) {
    let wins = data.reduce((accumulator, currentValue) =>{
        if (teamInitials === currentValue["Home Team Initials"] && currentValue["Home Team Goals"] > currentValue["Away Team Goals"]){
            return accumulator += 1
        }
        else if (teamInitials === currentValue["Away Team Initials"] && currentValue["Away Team Goals"] > currentValue["Home Team Goals"]){
            return accumulator += 1
        }
    } ,0)
    //return wins;
